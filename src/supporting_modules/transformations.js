...
export function localToGlobalCoord(coord) {
  ...
  return {
    x: document.documentElement.clientWidth
       * (coord.x - winSize.minX)
       / (winSize.maxX - winSize.minX),
    y: document.documentElement.clientHeight
       * (coord.y - winSize.minY)
       / (winSize.maxY - winSize.minY),
  }
}
...
export function globalToLocalCoord(coord) {
  const winSize = Globals.getWinSize()
  return {
    x: coord.x / document.documentElement.clientWidth * (winSize.maxX - winSize.minX)
       + winSize.minX,
    y: coord.y / document.documentElement.clientHeight * (winSize.maxY - winSize.minY)
       + winSize.minY,
  }
}
