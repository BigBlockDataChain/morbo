...
export function start() {
  if (Trans.getTransform(Globals.getGraph()) === null
      || Trans.getTransform(Globals.getGraph()) === undefined)
...
export function update() {
  const zoomActions = () => {
    ...
    Globals.getNodeLabels()
      .selectAll('text')
      .attr('font-size', 16 / gTransform.k)
      .attr('x', d => d.x - d.content.length * 4 / gTransform.k)
      .attr('y', d => gTransform.k > 2 ? d.y : d.y - 10)

    WINSIZE.minX = -gTransform.x / gTransform.k
    WINSIZE.maxX = document.documentElement.clientWidth / gTransform.k
                   - (gTransform.x + 15) / gTransform.k
    WINSIZE.minY = -gTransform.y / gTransform.k
    WINSIZE.maxY = document.documentElement.clientHeight / gTransform.k
                   - (gTransform.y + 15) / gTransform.k
  }
